...
public class BaseSPTool {
	...
	private static String SP_NAME = "clouddelivery";
	...
	public static void putString(Context c, String key, String value) {
		SharedPreferences sp = c.getSharedPreferences(SP_NAME,
				Context.MODE_PRIVATE);
		Editor editor = sp.edit();
		try {
			if (value == null) {
				editor.putString(key, encrypt(masterPassword, ""));
			} else {
	...
	public static long getLong(Context c, String key, long defalut) {
		SharedPreferences sp = c.getSharedPreferences(SP_NAME,
				Context.MODE_PRIVATE);
		try {
			return sp.getLong(key, defalut);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return -1l;
